                    ...
                    if(messageController.getChatModel().get(keySet).get(k) instanceof ProcessMessage && ((ProcessMessage) messageController.getChatModel().get(keySet).get(k)).getFileId().get(0)== key.get(0) && ((ProcessMessage) messageController.getChatModel().get(keySet).get(k)).getFileId().get(1) == key.get(1)){
                        if(isAck){
                            if(receivedMap.get(key).size() == fh.getTotalPackets(data)){
                                ...
                                messageController.setMessage(keySet, k, "Outgoing file. Received " + receivedMap.get(key).size() + "/" + fh.getTotalPackets(data) + ". (" + (Math.round(((double)receivedMap.get(key).size() / (double)fh.getTotalPackets(data))*100) + "%)"));
                            }
                        }else{
                            if(receivedMap.get(key).size() == fh.getTotalPackets(data)){
                                messageController.setMessage(keySet, k, "Transfer complete!");
                                ...
                                messageController.setMessage(keySet, k, "Incoming file. Sent " + receivedMap.get(key).size() + "/" + fh.getTotalPackets(data) + ". (" + (Math.round(((double)receivedMap.get(key).size() / (double)fh.getTotalPackets(data))*100) + "%)"));
                            }
                        }
                    }
                }
